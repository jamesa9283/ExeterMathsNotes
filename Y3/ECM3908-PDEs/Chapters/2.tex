...
\noindent
We shall now introduce characteristic projections. Suppose that $u(x, y)$ is specified along some curve $\La$ in the $(x, y)$-plane we then have $u = u_0(s)$ when $x = x_0(s)$ and $y = y_0(s)$ where $s$ parameterises $\La$ in 3D, $(x_0(s), y_0(s), u_0(s))$ is our initial curve. Characteristics pass through this curve and they are tangent to $(a, b, c)$, so
